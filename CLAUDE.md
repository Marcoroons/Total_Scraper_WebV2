...
- **YouTube as a scrape platform** (URL / Profile / Comments only — NOT hashtag / KOL Finder). The format axis is Shorts / Videos / All Formats (vs IG's Reels / Images / All). Actors: `streamers/youtube-scraper` (main — handles channel feeds AND specific video URLs AND shorts in ONE run via per-type caps `maxResults` / `maxResultsShorts` / `maxResultStreams`), `streamers/youtube-comments-scraper`. `streamers/youtube-shorts-scraper` is wired in the YT actor dict as an optional fallback but NOT currently called.
  ...
  - **Unverified output field names** (mapped defensively via `_yt_first_present(*candidates)`): `viewCount`/`views`/`viewsCount`, `likes`/`likeCount`, `commentsCount`/`commentCount`, `duration`/`durationSeconds`/`lengthSeconds`, `subscriberCount`/`numberOfSubscribers`, `subtitles`/`transcript`/`captions`/`subtitleText`, `channelName`/`channelTitle`/`author`/`uploaderName`, `hashtags`/`tags`. Comments cap key sent as `maxComments` + `maxResults` + `commentsPerVideo` (actor ignores unrecognised keys). **Run a one-job smoke test and check the Railway logs to confirm which actually populate; extend the candidate lists in `_yt_map_video` if any field is missed.**
