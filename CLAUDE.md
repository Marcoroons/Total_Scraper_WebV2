...
- 2026-06-30 — **YouTube URL-stats cap fix**: Specific URLs (Video Stats) on YouTube was sending `_yt_caps(fmt, 1)` (`{0,1,0}` for Shorts URL, `{1,0,0}` for Video URL), routing through the channel-feed sampling helper. For a single direct URL the actor scrapes the one source we passed — the per-type caps just need to NOT block. The old config could silently return zero rows for a `youtu.be/...` short-link the actor classified as a Short (blocked by `maxResultsShorts:0`). Now sends `{1,1,0}` (allow either type, streams off) for single-URL scrapes. Channel-feed Profile Audit path still routes through `_yt_caps` — that's correct there (the cap drives per-type sampling within the feed).
