...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12" style={{ background: "#060c18" }}>
      <div className="w-full max-w-md">
        ...
        <div className="flex items-center justify-center gap-2.5 mb-7">
          ...
          <span className="text-base font-semibold" style={{ fontFamily: "Outfit, sans-serif", color: "#dde4f4" }}>
            Total Scraper Web
          </span>
        </div>

        {/* Tagline + intro — incorporated from the old sidebar */}
        <div className="text-center mb-7">
          ...
          <p className="text-sm leading-relaxed max-w-sm mx-auto" style={{ color: "#5a7294" }}>
        ...
        >
          ...
          <p className="text-sm text-muted-foreground mb-6">Sign in to your account</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 text-sm font-semibold rounded-lg transition-opacity hover:opacity-90 disabled:opacity-60 mt-2"
          ...
          {/* Feature list — incorporated from the old sidebar */}
          <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 mt-7">
            {FEATURES.map((f) => {
              const Icon = f.icon;
              return (
                <div key={f.text} className="flex items-center gap-2 text-xs" style={{ color: "#5a7294" }}>
                  <Icon className="w-3.5 h-3.5 flex-shrink-0" style={{ color: "#00c9ff" }} />
                  {f.text}
                </div>
              );
            })}
